...
Eén master, één `pipeline.py`, en per markt een profiel in `GTM/Campaigns/shift/markets/<code>/`:
...
Het oude `GTM/Campaigns/shift/master/dossiers/` blijft staan als rondelogboek. De organisatiedossiers die eruit te halen waren staan nu in `dossiers/orgs/`; wat erin achterbleef zijn procesnotities en afgevallen organisaties zonder record.
