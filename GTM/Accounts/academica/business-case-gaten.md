# Business case: Academica University of Applied Sciences
_Laatst bijgewerkt: 2026-09-24. Bronnen: transcript discovery 24-09-2026 (Wispr Flow, laatste kwartier niet opgenomen), Close-lead en opportunity, Gmail-thread "AI en de schoolleidersopleiding" (13-14 aug), web (vacature AI-onderwijsontwerper, NVAO M Excellent Teaching), debrief Dante in de chat 24-09._

**Fase:** Discovery, gate nog niet gehaald. Pijn en een deel van de spelers zijn er, maar why-now, besluitschema en budget ontbreken, en de bevestigingsmail met vervolgstappen is nog niet verstuurd. (Close zegt: Contacted, dat loopt juist achter op het bewijs, want de discovery is gevoerd.)
**Gesprekspartner(s):** Marjolein Looij, Directeur Opleidingen over alle geaccrediteerde opleidingen, **champion** (volgens Dante). Brengt de businesscase naar het MT. EB onbekend: het MT of de bestuurder Mirte van den Berg (Dante denkt de bestuurder, navragen bij Tjarko).

## Wat we weten en wat ontbreekt

### 1. Probleem
- ✓ Kernpijn in haar woorden: *"waar haal je die competente mensen vandaan? En hoe zorg je dat je die genoeg traint om ook weer te kunnen nakijken? Nou, dat is tijdrovend, kost heel veel geld."* (transcript 24-09)
- ✓ Oorzaak: applied science, dus de nakijker moet theorie én praktijk snappen. *"Als jij die stukken gaat nakijken... dan ga je helemaal nat."* (transcript 24-09)
- ✓ Formatief gebeurt maar in een deel van de modules en is arbeidsintensief: *"we zijn altijd blij als niet veel studenten er gebruik van maken"*. Het probleem schuift dan door naar het eindstuk. (transcript 24-09)
- ✓ Randvoorwaarden handmatig checken (woorden tellen, inhoudsopgave): *"dat is allemaal onzin"*. (transcript 24-09)
- ✓ Instellingsbreed: zij gaat over alle geaccrediteerde opleidingen, niet één. (transcript 24-09)
- ✓ Wat veranderde: snelle groei, opleidingen gekocht en opnieuw geaccrediteerd, ~100 studenten per jaar erbij. (transcript 24-09)
- ✗ GAT: wat het nu concreet kost als een nakijker afhaakt of een nieuwe ingewerkt moet worden (één voorbeeld uit het afgelopen jaar).

### 2. As-is in cijfers
- ✓ Studenten waarvoor nagekeken wordt: 300 tot 400 per jaar in- en uitstroom. Eerder in hetzelfde gesprek zei ze "400, 500". (mondeling, 24-09)
- ✓ Groei: ~100 per jaar erbij. (mondeling, 24-09)
- ✓ 8 opleidingen: 6 geaccrediteerd, 1 gecertificeerd (Scipion), Pabo in accreditatie. (mondeling, 24-09)
- ✓ Eén summatieve opdracht per module, 7 of 10 modules per opleiding over 2 jaar, ~3.000 woorden exclusief bijlagen. (mondeling, 24-09)
- ✓ Thesis: ~50 per jaar, ~8.000 woorden, vierogenprincipe. Modules: tweeogen, behalve bij twijfel. (mondeling, 24-09)
- ✓ Doorlooptijd: formatief 1 week, summatief 4 weken. (mondeling, 24-09; staat waarschijnlijk in de OER)
- ✓ Kalibratiesessies: twee keer per jaar. (mondeling, 24-09)
- ✓ Wie nakijkt: vooral interne medewerkers in dienst, gastdocenten (zzp) geven vooral les. Iedere nieuwe nakijker moet een BKE-traject door, waarna zijn beoordelingen worden herbeoordeeld. (mondeling, 24-09)
- ✓ Scope jaar 1: de eerste 150 studenten. (Dante, 24-09) Welke opleidingen dat zijn: onbekend.
- ✗ GAT: minuten per opdracht, formatief en summatief. Het belangrijkste ontbrekende getal.
- ✗ GAT: aantal nakijkers, en hoeveel nieuwe per jaar het BKE-traject doorlopen.
- ✗ GAT: uren per nieuwe nakijker aan BKE plus herbeoordeling.
- ✗ GAT: duur en aantal deelnemers per kalibratiesessie.
- ✗ GAT: aantal herkansingen (elke onvoldoende gaat *"het hele riooltje weer van vooraf aan"*).
- ✗ GAT: huidige uitgaven aan tools (Turnitin, Copilot) of inhuur voor nakijken.

### 3. Hun prioriteiten
- ✓ Groei van de geaccrediteerde tak: *"we hebben een toekomstmodel nodig"*. (transcript 24-09)
- ✓ Accreditatie: Pabo loopt, en ze verwacht dat *"consistentie van beoordeling"* bij de nieuwe accreditaties van de andere opleidingen nog *"rammelt"*. (transcript 24-09)
- ✓ AI staat op de agenda: vacature Junior Onderwijsontwerper met focus AI, inclusief toetsing die past bij een tijd waarin AI gewoon onderdeel is van leren. (academica-group.com, vacature, 1 sep 2026 tot feb 2028)
- ✓ B Corp-gecertificeerd. (transcript 24-09, B Lab)
- ✗ GAT: een prioriteit uit hun eigen strategie- of instellingsplan, met link. Die weegt voor het MT zwaarder dan een uitspraak.

### 4. To-be plus kostenonderbouwing
- ✓ Gewenste to-be in haar woorden: formatief *"helemaal moet gewoon door jullie worden nagekeken"*, met steekproefsgewijze regie en analyse van waar studenten de opdracht niet snappen. (transcript 24-09)
- ✓ Summatief: Eduface als derde beoordelaar naast het tweeogenprincipe, om sneller consensus en kalibratie te bereiken. Door Dante voorgesteld, zij reageerde met "ja". (transcript 24-09)
- ✓ Kans die zij zelf aandroeg: nieuwe nakijkers laten beoordelen en hun oordeel naast de AI leggen in plaats van persoonlijk herbeoordelen. *"Omdat we het persoonlijk doen, kost het gewoon veel geld."* (transcript 24-09) Dit is geen bestaande Eduface-functie.
- ✗ GAT: verwachte minuten per opdracht na Eduface, en waar de vrijgekomen tijd naartoe gaat.
- ✗ GAT: de hele doorrekening. Kan pas als blok 2 cijfers heeft.
- Prijs (`GTM/Pricing/prijsmodel-psu-26-27.md`): 3 euro per student per maand, 12 maanden, **instellingsbreed**, drempel 300 lerenden. Twee open punten voor Dante en Tjarko: (1) scope van 150 studenten ligt onder de drempel; (2) telt "alle studenten" alleen de 300 tot 400 geaccrediteerde studenten of ook de 3.000 tot 6.000 trajectdeelnemers waarvoor niet wordt nagekeken?

### 5. Waarom nu
- ✓ Groei van ~100 studenten per jaar en de Pabo-accreditatie. (transcript 24-09)
- ✓ September is chaos door de studentenstart, daarom kon het gesprek pas nu. (transcript 24-09)
- ✗ GAT: een datum. Wanneer vergadert het MT erover, wanneer valt het budgetbesluit, wanneer start het volgende blok of cohort waarin je live wilt.
- ✗ GAT: wie tekent (MT of bestuurder).

### R. Risico's en ons antwoord
- ✓ AVG en data, haar vraag: *"wat doe je met je nieuwe klanten... hoe zit je met AVG?"* Mogen we zeggen: studentdata gaat nooit naar externe AI-aanbieders, geen training op hun data, de instelling blijft eigenaar. `[OPEN: AVG en verwerkersovereenkomst expliciet bevestigen bij Menno]`, al gaat de verwerkersovereenkomst nu mee in de mail.
- ✓ B Corp en milieu, haar woorden: *"Dat is heel ingewikkeld met AI en milieu."* `[OPEN: navragen bij Menno. Staat niets over in product.md, dus niets beweren]`
- ✓ LMS: NEO LMS, koppeling via API gewenst, geen integratie met het SIS (Progress) nodig. `[OPEN: technische validatie Samuel met Joey]`
- ✗ GAT: acceptatie door de nakijkers zelf, en wat de examencommissie en kwaliteitsmanager van AI in summatief beoordelen vinden.

## Metrics

| Metric | Waarde | Herkomst | Ladder | Sterker door |
|---|---|---|---|---|
| Studenten in scope | 300 tot 400 per jaar (ook "400, 500" genoemd) | hun schatting, mondeling | CURRENT STATE | Laat het uit Progress komen: het aantal actieve inschrijvingen per opleiding. Dan verdwijnt de tegenstrijdigheid. |
| Groei | ~100 per jaar erbij | hun schatting, mondeling | CURRENT STATE | Koppel aan hun begroting of groeiplan, dan is het hun cijfer en niet een indruk. |
| Opdrachten per jaar | onbekend. Onze ruwe som: 300 tot 400 studenten × 3,5 tot 5 modules per jaar ≈ 1.000 tot 2.000 summatieve opdrachten plus herkansingen plus formatief | onze doorrekening, geen cijfer van hen | UNKNOWN | Vraag een NEO-export van het aantal inleveringen per module over het afgelopen jaar. |
| Theses | ~50 per jaar, 8.000 woorden, vier ogen | hun schatting, mondeling | CURRENT STATE | Uit Progress of de examencommissie halen. |
| Doorlooptijd | formatief 1 week, summatief 4 weken | mondeling, staat vermoedelijk in de OER | CURRENT STATE | Citeer de OER-regel, dan is het hun eigen document. |
| Kalibratie | 2 keer per jaar | mondeling | CURRENT STATE | Aantal deelnemers × duur erbij, dan wordt het uren. |
| Minuten per opdracht | onbekend | n.v.t. | UNKNOWN | Vraag naar de normering in hun taakbelasting, niet naar een schatting. Tot die er is: sectoranker 15 minuten per inzending, gelabeld als sectorcijfer. |
| Kosten nieuwe nakijker (BKE + herbeoordeling) | onbekend, "kost heel veel geld" | hun oordeel, zonder getal | UNKNOWN | Uren per nieuwe nakijker × aantal nieuwe per jaar. |

## Vragen voor het volgende gesprek (donderdag 1 oktober, businesscase samen maken)
_Aantal: vier cijfers per mail en vier vragen live. Marjolein is een warme champion die zelf cijfers deelt en zelf ideeën aandraagt, maar ze vroeg om tempo en zit vol, dus alleen wat de businesscase donderdag echt nodig heeft. Budget en besluitvorming komen later._

### Mail vooraf (in de vervolgmail)
1. Hoeveel schriftelijke opdrachten kijken jullie per jaar na voor die eerste 150 studenten, en binnen welke opleidingen? Dan rekenen we met jullie echte volume. Mogelijk te halen uit NEO LMS.
   gat: scope en volume (blok 2). anker: *"één per module... 7 modules in de 2 jaar, de andere heeft er 10"* (24-09)
2. Hoeveel uur rekent jullie normering voor het nakijken van één module-opdracht, en voor een formatieve ronde? Dat is de basis voor de tijdwinst. Staat mogelijk in jullie taakbelastingsmodel.
   gat: minuten per opdracht (blok 2). anker: *"een deel van je baan is uitvoer: training geven, nakijken"* (24-09)
3. Hoeveel nieuwe nakijkers deden vorig jaar het BKE-traject, en hoeveel uur begeleiding en herbeoordeling zit er per nakijker in? Zo nemen we de inwerkkosten mee.
   gat: kosten inwerken (blok 2 en 4). anker: *"omdat we het persoonlijk doen, kost het gewoon veel geld. Want het zijn gewoon uren."* (24-09)
4. Hoe lang duurt een kalibratiesessie, en met hoeveel mensen? Dan kan ik uitrekenen wat Eduface daar scheelt.
   gat: kalibratie-uren (blok 2), plus jouw toezegging uit het gesprek. anker: *"sowieso twee keer per jaar"* (24-09)

### Live in de meeting
**[nu]** "Je noemde [uren uit antwoord 2] per opdracht. Wat gebeurt er in een blok waarin je te weinig nakijkers hebt?"
  gat: pijn kwantificeren (blok 1), met het mailantwoord als anker
  anker: *"waar haal je die competente mensen vandaan?"* (24-09)
  door: "Wat heeft dat vorig jaar gekost, in uren of in uitgestelde cijfers?"

**[nu]** "Je zei dat consistentie van beoordelen bij de nieuwe accreditaties nog rammelt. Waar zie je dat nu aan?"
  gat: prioriteit uit hun eigen document (blok 3)
  anker: *"daar rammelt het nog wel op, vooral consistentie van beoordeling"* (24-09)
  wie weet het: Marjolein, anders de kwaliteitsmanager of examencommissie. Doorvraag: "Staat dat ergens op papier, in het NVAO-rapport of jullie kwaliteitsplan?"

**[nu]** "Je noemde B Corp en de milieukant van AI. Wat moet er in de businesscase staan om dat bij het MT af te dekken?"
  gat: grootste bezwaar (R). Ons antwoord eerst bij Menno ophalen.
  anker: *"Dat is heel ingewikkeld met AI en milieu"* (24-09)

**[nu]** "Als het MT hiernaar kijkt, waar kijken ze dan het eerst naar? En hoe is bijvoorbeeld Progress destijds besloten?"
  gat: besluitvorming en wie tekent (blok 5), indirect via gewoontes. Nooit per mail.
  anker: *"Progress hebben we nu 3 jaar"* (24-09)
  door: "Wanneer vergadert het MT daar de volgende keer over?"

**[later: pas na de techniekcheck van Samuel met Joey]** "Wat heeft jullie IT nodig om de koppeling met NEO goed te keuren?"
  gat: LMS-risico (R). wie weet het: Joey.

**[later: live, via Marjolein, en pas nadat Tjarko zegt waar je op stuurt]** "Als de businesscase klopt, is er dan ruimte om dit studiejaar te starten, of hoort het in de begroting van volgend jaar?"
  gat: budget (blok 5), met de "als"-formulering. Nooit per mail.
