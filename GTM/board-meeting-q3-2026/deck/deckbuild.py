"""Eén bron voor een Eduface-deck.
...
# Logo's. KLANTLOGO zet je per deck, zie SKILL.md.
LOGO_NAVY = 'assets/logo_navy.png'
LOGO_WIT  = 'assets/logo_white.png'
KLANTLOGO = 'assets/klant.png'
# Maat van het klantlogo in de voet. Zet dit per deck: elk logo heeft een eigen
# verhouding en een drieregelig lockup moet kleiner dan een woordmerk van een regel.
KLANT_W, KLANT_H = 130, 60
...
def foot(quote=None, src=None, dark=False, page=None):
    ...
    if src:
        out.append(text(M, 950 if quote else 915, 1320, 70, src, size=23,
    ...
    out += [pic(LOGO_NAVY, dx, 907, 170, 46),
            rect(dx + 210, 904, 1, 52, INK200),
            pic(KLANTLOGO, dx + 251, 930 - KLANT_H / 2, KLANT_W, KLANT_H)]
    if page:
        out.append(text(1700, 1004, 100, 36, str(page),
                        size=23, color='FFFFFF73' if dark else '9AAFB8', align='right'))
    return out
...
def render_html(els, path, dark=False, prefix='../'):
    ...
    fonts_css = (
        ...
        f"@font-face{{font-family:'Inter';font-weight:400;"
        f"src:url(data:font/ttf;base64,{_font('assets/Inter-Regular.ttf')}) format('truetype')}}"
    )
    open(path, 'w', encoding='utf-8').write(
        '<!DOCTYPE html><html lang="nl"><head><meta charset="utf-8">'
        f'<style>{fonts_css}*{{margin:0;padding:0;box-sizing:border-box}}html,body{{background:#fff}}'
...
def shoot(html_path, png_path):
    ...
    if im.size != (1920, 1080):
        im.crop((0, 0, 1920, 1080)).save(png_path)
    return png_path
