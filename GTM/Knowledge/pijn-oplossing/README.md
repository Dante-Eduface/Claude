...
Eén pagina met alles wat prospects en klanten in Close hebben gezegd over hun pijn, gewenste uitkomst, bezwaren en huidige aanpak, per thema, met onze oplossing eronder (alleen claims uit `Platform/Product/product.md`).

## Bestanden
- `pijn-oplossing.html` — de pagina. Open via `node Design/Website/website-building/serve.mjs` vanuit de repo-root en ga naar `http://localhost:3000/GTM/Knowledge/pijn-oplossing/pijn-oplossing.html` (of dubbelklik, werkt ook zonder server). Dante beoordeelt hier per uitspraak: klopt (y) of weg (n), met notitie (o). Knop "Kopieer besluiten" zet zijn besluiten als JSON op het klembord.
...
## Verversen
```bash
python3 GTM/Knowledge/pijn-oplossing/close_alles.py GTM/Knowledge/pijn-oplossing/close-dump   # nieuwe Close-data (slaat al gedumpte leads over; verwijder _index.tsv voor een volledige refresh)
python3 GTM/Knowledge/pijn-oplossing/build.py                                            # pagina + md opnieuw
```
