"""Eén bron voor het RUG-deck (skeleton overgenomen van Windesheim).
...
def foot(quote=None, src=None, dark=False, page=None):
    """Footer met Eduface-logo + 'Rijksuniversiteit Groningen' als tekstlabel
    (geen RUG-beeldmerk beschikbaar, dus bewust tekst i.p.v. een gefabriceerd logo)."""
    ...
    out += [pic('logo_navy.png' if not dark else 'logo_white.png', dx, 907, 170, 46),
            rect(dx + 210, 904, 1, 52, INK200 if not dark else 'FFFFFF33'),
            text(dx + 231, 903, 270, 50, 'Rijksuniversiteit Groningen', size=20,
                 color=(INK500 if not dark else '8FA9B4'), align='left', ls=1.2)]
    if page:
...
def full_bleed(path):
    """Een volledige, al-kant-en-klare afbeelding (bv. hergebruikte echte
    krantenkoppen of een webinar-screenshot) die de hele slide vult."""
    return [pic(path, 0, 0, 1920, 1080)]
...
def shoot(html_path, png_path):
    subprocess.run([CHROME, '--headless', '--disable-gpu', f'--screenshot={png_path}',
                    '--window-size=1920,1080', '--hide-scrollbars', '--no-sandbox',
