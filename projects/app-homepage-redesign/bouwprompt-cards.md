...
**De volgorde van het raster ís de rangschikking.** In een tabel klik je op een kolomkop om te sorteren. In een raster is er niets om op te klikken, dus de standaardvolgorde moet zelf het antwoord zijn op "waar moet ik nu heen". Zie punt 7.
...
- **De voortgangsring.** Hij codeert proportie, en proportie is niet de beslissing. De docent vraagt zich af hoeveel er nog ligt en hoe dringend, niet hoe ver hij procentueel is. De ring trekt het meeste gewicht naar het minst bruikbare getal. Er komt ook geen balk of meter voor terug. De card moet rustig zijn, en de rangschikking loopt via de volgorde van het raster.
...
Wachten er inzendingen, dan één held-getal met daaronder één gewone regel:

```
20 to grade
Oldest waiting 12 days ⚠
```

`20 to grade` is het enige grote getal op de card. Het getal en zijn woord staan op dezelfde regel: dat lost op dat een card geen kolomkop heeft, want het label reist mee met het getal.

`Oldest waiting 12 days` staat eronder op gewone tekstgrootte, en is **het enige element op de card dat kleur mag krijgen**. Rood plus het waarschuwingsteken alleen wanneer het oudste wachtende werk voorbij de deadline van zijn eigen opdracht ligt. Dat is een feit van de instelling, geen drempel die wij verzinnen.

**Waarom `to grade` de held is en niet `oldest waiting`.** Een docent weet zelf ook wel dat hij nog moet nakijken. Een groot rood getal dat hem daar elke dag aan herinnert is een schuldgevoel, en een schuldgevoel op dagelijkse frequentie wordt behang. De attentiewaarde slijt, de irritatie niet. `oldest waiting` blijft wel de sorteersleutel uit punt 7: de rangschikking loopt via de volgorde van het raster en via die ene rode regel, niet via grootte.
...
### De typografie ligt vast: precies drie groottes

Niet vier, niet vijf. De huidige card zit op vijf en dat is waarom hij schreeuwt.

| Rol | Grootte | Gewicht | Font |
|---|---|---|---|
| Cursusnaam | 17px | 600 | Inter |
| Het held-getal | 24px | 600 | Inter |
| Al het andere | 14px | 400 | Inter |

**Geen League Spartan op de card.** De huidige versie zet de cursusnaam in het display-font op koptekstformaat. Dat is typografie van het web-oppervlak, en `internal/starten.md` zegt letterlijk dat je uit `web/` geen hero en geen grote koppen meeneemt naar een interne tool, en dat de dichtheid van ons moet zijn en niet van de componentbibliotheek. Een card in een dashboard is geen marketingpagina.

**Het getal is groter dan de titel, en dat is opzet.** Gewicht volgt wat verandert, niet wat identificeert. Een docent heeft maximaal zeven cursussen en kent de namen. De naam is elke dag hetzelfde, het getal is elke dag anders. Nieuwe informatie verdient het gewicht, een naamplaatje niet.

**Bij zeven cursussen doet positie het werk waarvoor je anders grootte nodig had.** Ze passen allemaal tegelijk in beeld, dus staat `to grade` op elke card op exact dezelfde plek, dan loopt het oog dat punt in één beweging langs alle zeven. Objecten op de verwachte plek worden 300 ms tot ruim 2 seconden sneller gevonden ([Roth e.a., IJHCS 2013](https://www.researchgate.net/publication/236630044)). Dat is het kolom-scannen dat je met cards zou verliezen, teruggewonnen met uitlijning in plaats van met volume.

Als alles schreeuwt, schreeuwt er niets. Precies één ding op dit scherm mag luid zijn, en dat is de cursus die over zijn deadline is.

### Waarom dit zo vastligt

Vastgesteld op zes criteria, met de antwoorden erbij:

1. **Trieëren of navigeren?** Trieëren. De getallen leiden, de titel is identiteit.
2. **Hoeveel cursussen?** Maximaal zeven. Ze passen in beeld, dus vergelijken kost geen volume.
3. **Hoe vaak?** Dagelijks in een nakijkperiode. Wat je dagelijks ziet moet zacht zijn.
4. **Wat is duurder als je het mist?** Een interface die te hard praat. Een docent weet zelf ook wel dat er werk ligt.
5. **Hoeveel hiërarchieniveaus?** Twee tot drie, de vaste regel.
6. **Waar komt de luidheid vandaan?** Uit de layout van de card, niet uit de inhoud.

Criterium 1 en 4 lijken elkaar tegen te spreken en doen dat niet: bij zeven cards lost positie de triage op, dus je kunt tegelijk rangschikken en stil zijn.

**Controleren na een echte nakijkperiode:** staan er dan vier of vijf van de zeven rood, dan klopt de drempel niet en werkt het signaal niet meer. Nu niets anders bouwen, wel over een periode meten.
...
- **Education level.** Staat niet op de card en is ook geen filter. Het wordt gezet in Manage rubric omdat het de toon en het niveau van de feedback stuurt, maar het verandert niet op welke cursus je gaat handelen.

## 9. Archiveren en verwijderen
...
## 10. Wat blijft gelden uit het eerdere werk
...
Het bijgewerkte bestand, en drie regels over wat je hebt gewijzigd en waar je van deze prompt bent afgeweken.
