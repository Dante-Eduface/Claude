# Eduface

_Laatst bijgewerkt: 2026-09-24._

Het bedrijf waar Dante werkt. Juridisch heet het **Blockbook B.V.**, daar komt zijn salaris ook vandaan. Naar buiten is alles Eduface. Dit bestand is alleen het minimum dat elke sessie nodig heeft: wat we verkopen, aan wie, en wie er werkt. Alles wat dieper gaat staat in een eigen bestand, zie **Waar de details staan**.

## Wat Eduface doet

Een onderwijskundig AI-platform voor beoordelen en feedback in het hoger onderwijs. Vier modules, twee live en twee in beta:

- **AI Paper Grader** (live). Feedback per concept tijdens het schrijven plus volledige rubric-scoring met uitlegbare cijferopbouw. Zes vakmodellen (Law, Economics, Social Sciences, STEM, Humanities, Health Sciences), dus geen generiek model op elk vak.
- **AI Exam Grader** (live). Open en gesloten antwoorden nakijken op schaal, cijfer terug naar het gradebook van het LMS.
- **Academic Integrity** (beta). De student verantwoordt zich mondeling over zijn eigen paper.
- **Oral Examination** (beta). Mondelinge afname met realtime transcriptie en beoordeling.

De docent keurt elke opmerking en elk cijfervoorstel goed voordat de student iets ziet. Integreert met Moodle, Blackboard, Brightspace, Canvas en andere systemen. Voldoet aan de AI Act.

Het is dus meer dan schriftelijke feedback en nakijken: mondeling toetsen hoort er ook bij.

## Twee versies van het product

- **Enterprise.** De versie waar onze betalende klanten op zitten. Hier geldt het NPS-doel van boven de 50 uit `Context/current-priorities.md`.
- **PLG** (product led growth). De versie waarop mensen via onze website zelf gratis een account aanmaken. Telt niet mee in de Enterprise-NPS.

Noem je een cijfer of een klantervaring, zeg er altijd bij welke van de twee je bedoelt. Ze door elkaar halen maakt elk getal waardeloos.

## Aan wie we verkopen

Universiteiten en hogescholen, én particuliere onderwijsinstellingen. Markten: Nederland, Verenigd Koninkrijk, Verenigde Staten.

## Wie er werkt

| Wie | Rol |
|---|---|
| **Jeroen van Gessel** | CEO en medeoprichter. Strategie, sales, fundraising, alles extern. Dante's belangrijkste counterpart. |
| **Menno Hahury** | CTO en medeoprichter. Bouwt het product. |
| **Samuel Rafini** | Senior Engineer. Bouwt het product. |
| **Tjarko Kwee** | Sales adviseur en angel investor in Eduface. Wekelijkse sales sessie met Jeroen en Dante. |
| **Dante Torbed** | Customer Success Manager. Zie `Context/me.md`. |

Wie eigenaar is van een deal verschilt per deal en staat in Close, bij de opportunity. Niet hier bijhouden.

Het team communiceert over **WhatsApp**.

## Tools

**Close** (CRM) · **Lemlist** (cold outreach) · **Google Workspace** (Gmail, Agenda, Drive) · **Framer** (eduface.me) · **Todoist** · **LinkedIn** (outreach, geen ads meer) · **Apple Notes**

Aangesloten als MCP in Claude Code: Gmail, Google Calendar, Google Drive, Close, Clay, Framer, Todoist, Lemlist.

## Waar de details staan

Niet elke sessie lezen, wel altijd naar verwijzen in plaats van uit het hoofd beweren:

- **Product, wat je wel en niet mag claimen:** `Platform/product.md`. Bevestigd door Dante op 19-09-2026 als dé bron voor elke productclaim. Staat het daar niet, dan beweer je het niet.

- **Prijs:** `GTM/Pricing/prijsmodel-psu-26-27.md`. 3 per student per maand in de valuta van de markt, drempel 300 lerenden. Nooit een prijs uit het hoofd noemen, altijd dit bestand lezen.
- **Verwerkersovereenkomst (DPA):** `GTM/Knowledge/verwerkersovereenkomst/`. Daar werken we mee (24-09-2026). Vraagt iemand erom, dan sturen wij onze eigen versie mee als bijlage. Dat is onze taak, niet die van de klant.
- **Salesproces en gates:** `GTM/Knowledge/sales-handbook-v1.md`, `GTM/Knowledge/meddpicc-states-and-gates.md`.
- **Pipeline en dealstand:** Close, niet een bestand hier.
