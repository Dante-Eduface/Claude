...
description: >-
  ...
  request for a premium scroll-scrubbed animated site or section. Trigger ook op het
  Nederlands dat Dante gebruikt: "bouw een website", "maak een scroll-site", "bouw een
  animated landingspagina", "maak er een filmische pagina van", "zet een 3D-video in de
  site", "scrollytelling", "bouw de campagnepagina". NOT for slide decks / HTML explainers
  or static brochure sites.
---
...
Dit is een externe skill, ingepast in Dante's repo op 15-09-2026. Waar deze laag botst met
de generieke tekst hieronder, wint deze laag.

### Voor je begint
1. `zsh .claude/skills/scroll-film-studio/scripts/doctor.sh` draaien. Lane A is groen op deze
...
3. Fonts: het merk-font uit `core/`, secundair Inter. De generieke tekst hieronder zegt
   "reach for expressive display faces" — dat geldt alleen voor een niet-Eduface merk.
...
- **Poort 2 (proef)** = één onderdeel af bouwen, niet de hele pagina. Bij een scroll-film is
  dat de hero plus het eerste hoofdstuk van de scroll. Akkoord ophalen.
- **Poort 3 (uitrol)** = de rest van de pagina, verify, copy-gate.
Dante kan poorten overslaan door "gewoon bouwen" te zeggen. Standaard staan ze aan.
...
margins op children, `framer-motion` als enige animatielib. Dat botst met GSAP/Lenis uit
Lane A. Kies bewust en zeg het hardop:
- **Blijft lokaal of wordt een losse pagina/Artifact** → GSAP + Lenis, volle Lane A.
- **Moet in Framer** → `framer-motion` + `useScroll`, geen GSAP, en lees `web/framer.md`.
Vraag dit in het interview (STEP 0, vraag 7 "waar gaat het live") en behandel het antwoord
als een technische keuze, niet als een detail.

### Kleine dingen die hier anders zijn
- Deploy gaat **niet** via Vercel. Eduface publiceert via Framer, of het blijft lokaal.
