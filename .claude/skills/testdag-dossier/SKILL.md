...
## Wat er in dit account al staat

- **Bouwer:** `GTM/Accounts/breederode-hogeschool/testdag/build.py`. Draaien met `python3 build.py data-<vak>.py`, levert `out/<vak>-mapje.html` en `.pdf` op A4, doorlopend genummerd, met de blanco formulieren van de andere vakken achterin.
- **Data per vak:** `data-mmt.py` is het werkende voorbeeld. Kopieer dat bestand voor een nieuw vak, de structuur is zelfbeschrijvend.
- **Fonts:** `testdag/assets/fonts.css` bevat League Spartan en Inter als ingebakken base64. Niet weghalen, anders valt de PDF terug op een systeemfont.
- **Rendering:** headless Chromium op `/opt/pw-browsers/chromium-1194/chrome-linux/chrome`. Geen Tailwind-build nodig, de tokenwaarden staan in `build.py` gelijk aan `Design/System/core/tokens.css`.
