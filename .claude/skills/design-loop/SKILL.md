---
name: design-loop
description: Takes a goal and a real-world reference, extracts what actually makes the reference good, then runs builder and three fresh-context critics on every piece until all three agree ours wins. Triggers on "/design-loop", "design loop", "run the critic loop", "loop this against", "vergelijk dit met", "laat dit langs de critici".
---

## EDUFACE-LAAG — lees dit eerst, het gaat voor op de rest van dit bestand

- Waar hieronder `design-system.md` staat, lees je `Design/System/core/` plus het oppervlak dat je bouwt (`web/`, `slides/` of `internal/`). Nooit de hele map. De system critic toetst tegen `core/regels.md` en `core/compositie.md`.
