...
1. `references/brand-guidelines.md` — kleuren, fonts, logo-regels, toon.
2. `references/design-system/tokens.css` — exacte design-tokens (kleur, type-schaal, radius, schaduw). **Verzin nooit losse waarden, lees ze hier.**
3. `reference.md` (in deze skill-folder) — slide-designpatronen, render-methodes, en gotchas.

Brand-assets (logo's, product-shots, fonts) staan centraal in `references/brand-assets/`. Gebruik altijd die, niet kopieën uit een project.
...
- **Logo:** `references/brand-assets/logos/logo_navy.png` (op licht) + `logo_white.png` (op navy/donker). Niet roteren/stretchen/herkleuren, houd clear space.
- **Product-shots:** `references/brand-assets/product/` (app, grading, criteria).
...
- **Paid ads** blijven in `projects/paid-ads-buildout/` — die structuur niet verplaatsen.

## De loop
```
1. SCOPE    wat, welk formaat/afmeting, projectgebonden of los? (1 vraag max, anders gewoon doen)
2. BASIS    lees brand-guidelines + tokens.css + reference.md, pak assets uit references/brand-assets/
...
- **PIL / Python** — pixel-perfecte ads/banners met exacte controle. Werkend voorbeeld: `projects/paid-ads-buildout/creatives/render_png.py` (helpers voor cards, schaduw, logo, glow, tekst-wrap, multi-formaat export).
