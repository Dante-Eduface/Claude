...
- Match de taak expliciet op een bestaande skill/agent (bijv. persoon onderzoeken → `person-research`, batch personen → `shift-research`/agent 3, pijplijn-berichten → `outreach`/agent 4, losse mails buiten de pijplijn → `targeted-outreach`, enz.).
- **SHIFT-pijplijn: de markt gaat altijd mee.** De keten is `lead-sourcing` (1) → `contact-sourcing` (2) → `shift-research` (3) → `outreach` (4) → `lemlist-import` (5), en elk van die vijf leest het marktprofiel in `projects/shift/markets/<code>/`. Noemt Dante geen markt, vraag het in één regel; geef de markt door aan elke subagent die je aanroept (`--markt <code>`). De oude `-nl`-skills zijn sinds 10-09-2026 vervangen en alleen nog een verwijzing.
