...
STATUSBESTAND="$PROJECT/GTM/Campaigns/shift/NACHTRUN.md"
...
(( DROOG )) || git -C "$PROJECT/GTM/Campaigns/shift" pull --quiet --rebase 2>>"$LOG" \
...
STAP 1, de peiling. Lees de campagne-id uit GTM/Campaigns/shift/markets/$MARKT/profiel.json
...
if (( DROOG )); then
  ...
  print "  git add, commit en push in GTM/Campaigns/shift"
  print "  stand naar GTM/Campaigns/shift/NACHTRUN.md"
  print ""
...
git -C "$PROJECT/GTM/Campaigns/shift" add -A >>"$LOG" 2>&1
git -C "$PROJECT/GTM/Campaigns/shift" commit -q -m "Nachtronde $(date '+%d-%m-%Y'), markt $MARKT" >>"$LOG" 2>&1 \
  && git -C "$PROJECT/GTM/Campaigns/shift" push --quiet >>"$LOG" 2>&1 \
