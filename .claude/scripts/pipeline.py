...
Twee tabellen in GTM/Campaigns/shift/master/ (sinds 10-09-2026 internationaal, de markt is
een variabele: --markt <code>, SHIFT_MARKT, anders nl. Profielen in GTM/Campaigns/shift/markets/):
...
# Twee geldige lay-outs voor dezelfde pipeline.py: genest onder GTM/Campaigns/shift/
...
# maakte het bericht er de last van de DOCENT van. Zie Platform/Product/onderwijs-vaktermen.md.
...
# staat in GTM/Campaigns/shift/markets/<code>/: profiel.json voor de harde waarden
...
def eis_profiel_compleet():
    ...
    if fouten:
        die("markt '%s' is niet compleet, dus geen wachtrij:\n  - %s\n"
            "Vul GTM/Campaigns/shift/markets/%s/ aan en controleer met: pipeline.py --markt %s doctor"
    ...
    for s in secties:
        ...
        if s == "Citaten":
            regels += [
                ...
                "   GTM/Campaigns/shift/markets/%s/profiel.md. Lees ze nooit als gewone taal. -->" % MARKT,
            ]
        regels.append("")
    return "\n".join(regels).rstrip() + "\n"
