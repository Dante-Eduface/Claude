...
Opbouw van de bron: '# titel', een bronregel, '## Exercise ...' per opgave,
en per vraag een 'Q:'-regel met de letterlijke vraag en een 'A:'-regel met het modelantwoord.
...
def lees(pad):
    ...
    for regel in pad.read_text(encoding="utf-8").splitlines():
        ...
        elif regel.startswith("Q:"):
            opgaven[-1][1].append([regel[2:].strip(), ""])
        elif regel.startswith("A:"):
            opgaven[-1][1][-1][1] = regel[2:].strip()
    return titel, bron, opgaven
...
def bouw(pad):
    titel, bron, opgaven = lees(pad)
    delen = []
    for opgave, vragen in opgaven:
        code = opgave.replace("Exercise ", "")
        delen.append(f"<h2>{html.escape(opgave)}</h2>")
        for nr, (vraag, antwoord) in enumerate(vragen, 1):
            delen.append(
                f'<div class="vraag"><p class="label">Question {code}.{nr}</p>'
                f"<p>{html.escape(vraag)}</p>"
                f'<p><span class="label">Model answer:</span> {html.escape(antwoord)}</p></div>'
            )
    htmlpad = pad.with_suffix(".html")
    htmlpad.write_text(
        SJABLOON.format(titel=html.escape(titel), bron=html.escape(bron), inhoud="\n".join(delen)),
        encoding="utf-8",
    )
